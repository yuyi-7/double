...
class JsnySpider(scrapy.Spider):
    nema = '江苏农业职业技术学院'
    allowed_domains = ['jsafc.edu.cn']
    start_urls = ['http://jsafc.91job.gov.cn/news/index/tag/tzgg']
    ...
    def parse(self, response):
        ...
        for i in range(len(title)):
            if (title[i].find("招聘会") != -1 or title[i].find("双选会") != -1 or title[i].find("宣讲会") != -1)  and time == publishDate[i]:
                ...
                item['url'] = 'http://jsafc.91job.gov.cn'+url[i]
                yield item
            else:
                print('没有匹配')